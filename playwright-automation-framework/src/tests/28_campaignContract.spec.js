...
const { CampaignContractPage } = require("../pages/campaignContractPage");
...
test("Verify that user is able to navigate to campaign contract page and see the contract details", async ({ page }) => {
  const loginPage = new LoginPage(page);
  const campaignPage = new CampaignPage(page);
  const campaignOverviewPage = new CampaignOverviewPage(page);
  const campaignContractPage = new CampaignContractPage(page);
  ...
  try {
    ...
    // Step 4: Navigate to campaign contracts section

    await campaignContractPage.navigateToContractsSection();

    // Step 5: Verify the Campaign Contracts page
    const isContractsPageLoaded = await campaignContractPage.verifyContractsPageLoaded();
    expect(isContractsPageLoaded).toBe(true);
  } catch (error) {
    console.error("❌ Campaign Contracts Test failed:", error);
