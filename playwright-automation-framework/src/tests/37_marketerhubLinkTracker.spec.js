...
const { MarketerhubLinkTrackerPage } = require("../pages/marketerhubLinkTrackerPage");
...
test("Verify that user is able to navigate to reporting page, click link tracker tab and see the Link Tracker page", async ({ page }) => {
  const loginPage = new LoginPage(page);
  const campaignPage = new CampaignPage(page);
  const campaignOverviewPage = new CampaignOverviewPage(page);
  const marketerhubLinkTrackerPage = new MarketerhubLinkTrackerPage(page);
  ...
  try {
    ...
    await marketerhubLinkTrackerPage.navigateToReportingPage();

    // Step 5: Click on the Link Tracker tab
    await marketerhubLinkTrackerPage.clickLinkTrackerTab();

    // Step 6: Verify the Link Tracker page is loaded
    const isLinkTrackerPageLoaded = await marketerhubLinkTrackerPage.verifyLinkTrackerPageLoaded();
    expect(isLinkTrackerPageLoaded).toBe(true);

    // Step 7: Verify the specific heading "You have no links created for this campaign"
    const isCorrectHeading = await marketerhubLinkTrackerPage.verifyLinkTrackerHeading();
